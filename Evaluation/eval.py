...
from Implementation.answer import answer_question, fetch_context, answer_question_pure_llm
...
def evaluate_answer(test: TestQuestion, mode="RAG", full_kb="") -> tuple[AnswerEval, str, list, dict]:
    """
    Evaluate answer quality using LLM-as-a-judge (async).
    """
    print(f"DEBUG: evaluate_answer called with mode='{mode}' and KB length={len(full_kb)}")

    # Toggle between RAG and Pure LLM
    if mode == "PURE_LLM":
        generated_answer, retrieved_docs, usage_stats = answer_question_pure_llm(test.question, full_kb)
    else:
        generated_answer, retrieved_docs, usage_stats = answer_question(test.question)

    # 1. Get RAG response from your existing implementation
    generated_answer, retrieved_docs, usage_stats = answer_question(test.question)
...
Please evaluate the generated answer on three dimensions: Accuracy, Completeness, and Relevance.
Provide detailed feedback and scores from 1 to 5. If the answer is wrong, accuracy must be 1.""",
        },
    ]

    # 3. FIX: Added max_tokens=1000 to prevent JSON truncation (EOF error)
    ...
    try:
        ...
        # Fallback object so the loop doesn't crash
        answer_eval = AnswerEval(feedback=f"Error: {str(e)}", accuracy=1.0, completeness=1.0, relevance=1.0)

    # Return the 4 values needed by the loop
    return answer_eval, generated_answer, retrieved_docs, usage_stats
...
def evaluate_all_answers(mode="RAG", full_kb=""):
    """
    Evaluate all answers and yield progress with INDIVIDUAL query stats.
    This prevents the 6.5M token doubling error in Gradio.
    """
    tests = load_tests()
    total_tests = len(tests)
    
    for index, test in enumerate(tests):
        # Call the updated evaluate_answer
        result, gen_ans, docs, usage = evaluate_answer(test, mode=mode, full_kb=full_kb)
        
        progress = (index + 1) / total_tests
        
        # FIX: Yield 'usage' (individual stats) instead of 'cumulative_stats'
        # This allows your Gradio app to do the summing correctly.
        yield test, result, progress, usage
...
def run_cli_evaluation(test_number: int):
    ...
    answer_result, generated_answer, retrieved_docs, usage_stats = evaluate_answer(test)
    ...
    print(f"Tokens Used: {usage_stats.get('total_tokens')}")
    print(f"Cost: ${usage_stats.get('total_cost_usd'):.4f}")    
    print(f"\n{'=' * 80}\n")

    print(f"\n{'=' * 40}")
    print(f"📊 RAG SETUP PERFORMANCE SUMMARY")
    print(f"{'=' * 40}")
